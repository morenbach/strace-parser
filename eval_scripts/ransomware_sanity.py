...
def sanityTest(username, password, test_prog, idx):
    base_test = os.path.basename(test_prog)
    outputFilePath = "/home/morenbach/dummy.log"
    vmiOutputFileName = '/home/morenbach/eval/vmi_logs_%s/vmi_%d.txt' % (base_test, idx)
    straceOutputFileName = '/home/morenbach/strace_logs_%s/strace_%d.txt' % (base_test, idx)
    print ("Starting test %s...storing file to: %s" % (base_test, vmiOutputFileName))		
    GuestAddress = "192.168.122.185"

    outputFile = open(outputFilePath,'wb')

    vmi_pid = Popen("nice -19 /home/morenbach/kvm-vmi/libvmi/build/examples/vmi-syscalls-trace -n ubuntu_meni -j /home/morenbach/Ubuntu-5.8.0-41.json -s /tmp/introspector -m 500 -o {}".format(vmiOutputFileName).split()).pid
    ...
    if not s.login (GuestAddress, username, password):
        ...
        s.sendline("LD_PRELOAD=/home/administrator/syscall_test/hax.so strace -ttt -ff -o {} {}".format(straceOutputFileName, test_prog))        
...
def runInternalTest(user, password, test_prog, iterations):
    ...
    if not s.login (GuestAddress, user, password):
        ...
        print ("SSH session login successful")
        s.sendline("rm -rf /home/morenbach/strace_logs_{}".format(base_test))
        s.sendline("mkdir -p /home/morenbach/strace_logs_{}".format(base_test))
        s.logout()
...
def runTest():
    ...
    base_test = os.path.basename(args.test)
    logs_dir = "/home/morenbach/eval/vmi_logs_" + base_test
